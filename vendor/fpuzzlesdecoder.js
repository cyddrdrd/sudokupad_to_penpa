/*
 * F-puzzles to SudokuPad translator by Sven Neumann.
 * Source: SudokuPad/sudokutools, commit f538286431c7b9b2741dba2f6160665c167d4d88.
 * This Source Code Form is subject to the terms of the Mozilla Public
 * License, v. 2.0. See MPL-2.0.txt or https://mozilla.org/MPL/2.0/.
 * Local changes: parsed-object input, fixed import ID (no MD5 dependency),
 * fractional/negative RC coordinates, multiline metadata, empty metadata cages, public feature list.
 */
/*
Unsupported:
*/
const loadFPuzzle = (() => {
	...
	const fpuzzlesParseRC = rc => rc.match(/R(-?\d+(?:\.\d*)?)C(-?\d+(?:\.\d*)?)/i).slice(1, 3).map(Number).map(n => n - 1);
	...
	const parseFPuzzle = fpuzzleRaw => {
		let fpuzzle = typeof fpuzzleRaw === 'string' ? saveDecodeURIComponent(fpuzzleRaw) : fpuzzleRaw;
		if(typeof fpuzzle === 'string') fpuzzle = JSON.parse(base64Codec.decompress(fpuzzle));
		let puzzle = {id: 'fpuzzle-import'};
	...
	loadPuzzle.supportedFeatures = Object.keys(parse);
...
if (typeof globalThis !== "undefined") globalThis.SudokuPadFPuzzles = loadFPuzzle;
